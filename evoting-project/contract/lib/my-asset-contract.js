...
class MyAssetContract extends Contract {
    ...
    async init(ctx){
        ...
        //create voters
        let voter1 = await new Voter('V1', 'first', 'voter');
        let voter2 = await new Voter('V2', 'second', 'voter');
        ...
        if (currElections.length === 0) {
            ...
            let electionEndDate = await new Date();

            
            electionEndDate.setDate(electionStartDate.getDate() +  1);

            //create the election
            election = await new Election('2020 Election', 'Banana Republic',
                '2020', electionStartDate, electionEndDate);
        ...
        let blueVotable = await new VotableItem(ctx, 'blue', 'I am Mr. Blue. And this is my agenda...');
        let redVotable = await new VotableItem(ctx, 'red', 'I am Mr. Red. And this is my agenda...');
        let greenVotable = await new VotableItem(ctx, 'green', 'I am Mr. Green. And this is my agenda...');
        let pinkVotable = await new VotableItem(ctx, 'pink', 'I am Miss. Pink. And this is my agenda...');
        let yellowVotable = await new VotableItem(ctx, 'yellow', 'I am Miss. Yellow. And this is my agenda...');

        //populate choices array so that the ballots can have all of these choices 
        votableItems.push(blueVotable);
        votableItems.push(redVotable);
        votableItems.push(greenVotable);
        votableItems.push(pinkVotable);
        votableItems.push(yellowVotable);
    ...
    */
    async createVoter(ctx, args) {
        ...
        //create a new voter
        let newVoter = await new Voter(args.voterId, args.firstName, args.lastName);
